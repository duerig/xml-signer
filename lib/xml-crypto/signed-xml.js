/*global: define: true */
define (['lib/xmldom/dom-parser', 'lib/xml-crypto/xpath',
         'lib/xml-crypto/utils', 'lib/xml-crypto/exclusive-canonicalization',
         'lib/xml-crypto/enveloped-signature', 'lib/forge/forge'],
function (domExport, select, utils, ExclusiveCanonicalization,
          EnvelopedSignature, forge) {

var Dom = domExport.DOMParser;
...
function SHA1() {
  ...
  this.getHash = function(xml) {    
    var sum = forge.md.sha1.create();
    sum.update(xml);
    var res = forge.util.encode64(sum.digest().getBytes());
//    var res = b64_sha1(xml);
/*
...
function SHA256() {
  ...
  this.getHash = function(xml) {
    var sum = forge.md.sha256.create();
    sum.update(xml);
    var res = forge.util.encode64(sum.digest());
    console.log('SHA256', res);
//    var res = b64_sha256(xml);
/*
...
function RSASHA1() {
  ...
  */
  this.getSignature = function(signedInfo, signingKey) {            
    var signer = forge.md.sha1.create();
    signer.update(signedInfo);
    var res = forge.util.encode64(signingKey.sign(signer));
//    var res = hex2b64(signingKey.signString(signedInfo, 'sha1'));
/*
...
*/
  ...
  */
  this.verifySignature = function(str, key, signatureValue) {
    var rsa = forge.pki.publicKeyFromPem(key);
    var hash = forge.md.sha1.create();
    hash.update(str);
    var res = rsa.verify(hash.digest());
/*    
    var rsa = new RSAKey();
    rsa.readPrivateKeyFromPEMString(key);
    var hexSig = b64tohex(signatureValue);
    var resInt = rsa.verify(str, hexSig);
    var res = false;
    if (resInt === 1) {
      res = true;
    }
...
function RSASHA256() {
  ...
  */
  this.getSignature = function(signedInfo, signingKey) {            
    var signer = forge.md.sha256.create();
    signer.update(signedInfo);
    var res = forge.util.encode64(signingKey.sign(signer));
//    var res = hex2b64(signingKey.signString(signedInfo, 'sha256'));
/*
...
SignedXml.prototype.computeSignature = function(xml) {  
  ...
  var sigRoot = doc.documentElement;
  if (this.signatureNode) {
    sigRoot = select(doc, this.signatureNode)[0];
  }
  sigRoot.appendChild(signatureDoc.documentElement)
...
exports.StoredKeyInfo = StoredKeyInfo;
return exports;
